...
PROMPT='Read Inner Loop2.txt for instructions on how to analyse. Pick a random text file from the Check2 folder to analyse. Add your result to classifications2.csv (Do not read this CSV before making your classification), and before adding the result be sure that you have made a classification for each round. Seperately, add (echo) the logical steps of your reasoning for each round of the classification to inner_loop_reasoning2.csv, be sure to mention the relevant filename at the start. Then move to the file to the checked folder, use the mv command to do this, not move. All slashes in the filepath should be / to function correctly. When you are finished, end your report with the phrase CLASSIFICATION FINISHED.'
...
while true; do
    ...
    files_list=$(ls -1 Check2 2>/dev/null | head -5)
