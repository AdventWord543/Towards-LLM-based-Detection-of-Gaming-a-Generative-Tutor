...
PROMPT='Read Inner Loop1.txt for instructions on how to analyse. Pick a random text file from the Check folder to analyse. Add your result to classifications.csv (do not read this CSV before making your classification), and before adding the result be sure that you have made a classification for each round. Seperately, add (echo) the logical steps of your reasoning for each round of the classification to inner_loop_reasoning1.csv, be sure to mention the relevant filename at the start. Then move the file to the checked folder, use the mv command to do this, not move. All slashes in the filepath should be / to function correctly. When you are finished, end your report with the phrase CLASSIFICATION FINISHED.'
...
while true; do
    ...
    # Check if there are files left in Check folder
    file_count=$(ls -1 Check 2>/dev/null | wc -l)
    files_list=$(ls -1 Check 2>/dev/null | head -5)

    if [ "$file_count" -eq 0 ]; then
        end_time=$(date "+%Y-%m-%d %H:%M:%S")
        echo "[$end_time] === No more files in Check folder. Done! ==="
        break
    fi
    ...
    output=$(claude -p --model claude-sonnet-4-5-20250929 --max-turns 100 "$PROMPT" 2>&1)
