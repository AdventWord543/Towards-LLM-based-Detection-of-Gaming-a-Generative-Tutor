...
PROMPT_TEMPLATE='Read "Outer Loop1.txt" for instructions on how to analyse. Analyse the file Check1/FILE_PLACEHOLDER. Add your result to classifications1.csv (do not read that csv before finishing your analysis). Seperately, add (echo) the logical steps of your reasoning for the classification to outer_loop_reasoning1.csv, be sure to mention the relevant filename at the start. Then move the file to the Checked1 folder, use the mv command to do this, not move. All slashes in the filepath should be / to function correctly. When you are finished, end your report with the phrase CLASSIFICATION FINISHED.'
...
while true; do
    ...
    current_file=$(ls -1 Check1 2>/dev/null | head -1)
