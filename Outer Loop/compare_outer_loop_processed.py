...
predicted_df = pd.read_csv('classifications_processed.csv')

# Rename columns for easier comparison
truth_df.columns = ['ID', 'Gaming_Truth', 'GCat_Truth', 'SCat_Truth']
predicted_df = predicted_df[['Filename', 'Gaming', 'Main Category', 'Subcategory']]  # Drop Success column
...
def extract_number(id_str):
    """Extract starting number (e.g., '01', '02', '16', etc.)"""
    return id_str.split('.')[0]

def extract_pattern(id_str):
    """Extract pattern number (e.g., '.1.', '.2.', etc.) - returns None for IDs like 16.1_A"""
    parts = id_str.split('.')
    if len(parts) >= 3:
        # Format like 01.1.1_A - has pattern
        return f".{parts[1]}."
    return None
...
output_dir = 'Category_Reports_Processed'
...
with open(overall_suffix_report, 'w', encoding='utf-8') as f:
    ...
    # Breakdown by suffix and pattern (only for items that have patterns)
    f.write("=" * 80 + "\n")
    f.write("SUFFIX BREAKDOWN BY PATTERN (Categories 01-14 only)\n")
    f.write("=" * 80 + "\n\n")
...
for num in numbers:
    ...
    with open(report_filename, 'w', encoding='utf-8') as f:
        ...
        # Breakdown by pattern (only if this category has patterns)
        has_patterns = num_df['Pattern'].notna().any()
        if has_patterns:
        ...
        for suffix in suffixes:
            ...
            if len(suffix_df) > 0:
                ...
                for _, row in suffix_df.iterrows():
                    ...
                    # Handle empty values
                    gt = str(row['Gaming_Truth']) if pd.notna(row['Gaming_Truth']) and row['Gaming_Truth'] != '' else '-'
                    gp = str(row['Gaming_Pred']) if pd.notna(row['Gaming_Pred']) and row['Gaming_Pred'] != '' else '-'
                    gct = str(row['GCat_Truth']) if pd.notna(row['GCat_Truth']) and row['GCat_Truth'] != '' else '-'
                    gcp = str(row['GCat_Pred']) if pd.notna(row['GCat_Pred']) and row['GCat_Pred'] != '' else '-'
                    sct = str(row['SCat_Truth']) if pd.notna(row['SCat_Truth']) and row['SCat_Truth'] != '' else '-'
                    scp = str(row['SCat_Pred']) if pd.notna(row['SCat_Pred']) and row['SCat_Pred'] != '' else '-'
...
with open(master_report, 'w', encoding='utf-8') as f:
    ...
    f.write("Range 01-14 (with patterns):\n")
    ...
    f.write("Range 16-19 (without patterns):\n")
    ...
    # Pattern breakdown only for items that have patterns
    f.write("By Pattern (Categories 01-14 only):\n")
    patterns = ['.1.', '.2.', '.3.', '.4.']
    for pattern in patterns:
